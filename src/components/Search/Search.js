    ...
    return (
      <>
        <form onSubmit={this.handleNewTodoKeyDown}>
          <input type="text" placeholder="Type to search..." value={value} onChange={this.handleChange} autoFocus />
        </form>
        {loading && <Loader />}
        {error && <ErrorIndicator description="There's been a mistake but we trying to fix it. Bear with us" />}
        {!loading && movies.length === 0 && (
          <ErrorIndicator description="We were unable to find a movie with that title. Please try again" />
        )}
        {!loading && movies.length > 0 && (
          <>
            <MovieCardList movies={movies} guestSessionId={guestSessionId} onMovieRate={onMovieRate} />
            <MoviesPagination current={currentPage} total={totalPages} onChange={this.handlePageChange} />
          </>
        )}
      </>
    );
